...
function animateMap() {
    ...
    var nextCenter = new google.maps.LatLng(
        start.lat() + (end.lat() - start.lat()) * step / steps,
        start.lng() + (end.lng() - start.lng()) * step / steps
    );
    map.panTo(nextCenter);
    setTimeout(animateMap, stepDuration);
}


document.querySelector(".up").addEventListener("click", () => {
    var dad = document.getElementById('map').style.top
    alert(dad)
    dad-=100
   document.getElementById('map').style.top= dad + 'px';
