...
document.querySelector(".openpopup").addEventListener("click", () => {
    ...
    for (let currentMonth = startMonth; currentMonth < endMonths; currentMonth++) {
        const numberOfDaysInMonth = new Date(currentYear, currentMonth + 1, 0).getDate();
        let currentDay = 1;
        while (currentDay <= numberOfDaysInMonth) {
            
           // calendardates.push('hh')
            currentDay++
            
        }
    }
    alert(calendardates)
    openCalendarWindow(currentYear, startMonth, endMonths)
...
function openCalendarWindow(currentYear, startMonth, endMonths) {
...
<th colspan="7">&nbsp;</th>
  ...
  `;
        ...
        while (currentDay <= numberOfDaysInMonth) {
            ...
            calendarHTML += `<td id='${currentYear}-${currentMonth + 1}-${currentDay}' onclick="saveDate('${currentYear}-${currentMonth + 1}-${currentDay}',${daterange})">${currentDay}</td>`;
...
calendarHTML += `
    <button onclick="closeCalendarWindow()">Close</button>
              <script>
        function saveDate(day,ff) {
            temp='2022-5-' 
            for (let i = 10; i < 22; i++) {
            document.getElementById(temp + i).style.backgroundColor = '#999999';
            }
